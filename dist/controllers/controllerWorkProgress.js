...
exports.getAllWorkProgress = getAllWorkProgress;
exports.getWorkProgressStats = getWorkProgressStats;
exports.postWorkProgress = postWorkProgress;
...
async function getWorkProgress(req, res) {
    try {
        ...
        const result = await database_1.default.query('SELECT * FROM WorkProgress WHERE UserID = $1', [userId]);
        if (result.rows.length === 0) {
            // If no progress record exists, create one with default values
            const createResult = await database_1.default.query('INSERT INTO WorkProgress (UserID, PendingTasks, CompletedTasks, InProgressTasks, TotalTasks) VALUES ($1, 0, 0, 0, 0) RETURNING *', [userId]);
...
async function postWorkProgress(req, res) {
    try {
        ...
        const result = await database_1.default.query('INSERT INTO WorkProgress (UserID, PendingTasks, CompletedTasks, InProgressTasks, TotalTasks) VALUES ($1, $2, $3, $4, $5) RETURNING *', [userId, pendingTasks, completedTasks, inProgressTasks, totalTasks]);
