...
async function getTotalRevenue(req, res) {
    try {
        ...
        const result = await pool.query(
            'SELECT SUM(Revenue) as total_revenue, COUNT(*) as total_sales FROM DailySales WHERE UserID = $1',
            [userId]
        );
...
async function postSale(req, res) {
    try {
        ...
        const result = await pool.query(
            'INSERT INTO DailySales (UserID, TemplateName, Revenue, SaleDate) VALUES ($1, $2, $3, $4) RETURNING *',
            [userId, templateName, revenue, saleDate || new Date().toISOString().split('T')[0]]
        );
