...
async function getSalesGrowth(req, res) {
    try {
        ...
        const result = await pool.query(
            'SELECT * FROM SalesGrowth WHERE UserID = $1 ORDER BY Timeline DESC LIMIT $2',
            [userId, limit]
        );
...
async function getLatestSalesGrowth(req, res) {
    try {
        ...
        const result = await pool.query(
            'SELECT * FROM SalesGrowth WHERE UserID = $1 ORDER BY Timeline DESC LIMIT 1',
            [userId]
        );
...
async function getSalesGrowthByDateRange(req, res) {
    try {
        ...
        const result = await pool.query(
            'SELECT * FROM SalesGrowth WHERE UserID = $1 AND Timeline BETWEEN $2 AND $3 ORDER BY Timeline DESC',
...
async function postSalesGrowth(req, res) {
    try {
        ...
        const result = await pool.query(
            'INSERT INTO SalesGrowth (UserID, USP, STP, Timeline) VALUES ($1, $2, $3, $4) RETURNING *',
            [userId, usp, stp, timeline || new Date().toISOString().split('T')[0]]
        );
...
export {
    getSalesGrowth,
    getLatestSalesGrowth,
    getSalesGrowthByDateRange,
    postSalesGrowth
};